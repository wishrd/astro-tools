...
<Aside type="note">Both integrations, `client:on` integration and this one, can be included in the project configuration, allowing you to use both directives, `client:viewport` and `client:on="viewport"`, in your components.</Aside>
...
<Aside type="tip">If you don't want to use this directive with `client:on`, it is recommended to use the Astro `client:visible` directive instead.</Aside>
