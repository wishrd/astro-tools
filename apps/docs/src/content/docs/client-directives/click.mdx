...
<Aside type="note">Both integrations, `client:on` integration and this one, can be included in the project configuration, allowing you to use both directives, `client:click` and `client:on="click"`, in your components.</Aside>
