...
import ClientDirectiveOnAndUseExample from './_examples/on/ExampleAnd.astro';
...
import clientDirectiveOnAndUseExampleRaw from './_examples/on/ExampleAnd.astro?raw';
...
In some use cases, you would want to meet all conditions before hydrating. For example, hydrating on visible but on a specific viewport size.

For this, wrap each group of directives between parenthesis `()` and join them with the `and` word:
```html
<Example client:on="(media (max-width: 740px)) and (click; timer 3000)" />
```

In the following example, we want to hydrate in mobile viewport AND after clicking:

<Example>
  <Tabs>
    <TabItem label="Example.astro">
      <Code code={clientDirectiveOnAndUseExampleRaw} lang="astro" />
    </TabItem>
    <TabItem label="Square.svelte">
      <Code code={squareCodeExample} lang="svelte" />
    </TabItem>
  </Tabs>
  <ExamplePreview hydration="example-and">
    <ClientDirectiveOnAndUseExample id="example-and" />
  </ExamplePreview>
</Example>
