...
Function of type `(path: string) => string` that transform the input file path, allowing to modify the output directory and filename.

The default function replaces `README` by `index`.
