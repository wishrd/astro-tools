...
Functions of type `(file: File, context: Context) => File[] | File` that transform the input file, allowing to modify the output path (including the file name) and the content (if available).
...
The default transformers replaces `README` by `index` and process the image assets hashing and copying it. Any custom transformer will be executed before the default transformers.
