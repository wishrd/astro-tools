...
The transformers option allows to declare groups of transformers as an array of arrays. For example:
...
Each group of transformers will get as context the properly `files` which has the processed files of the previous group. This way, transformers can be executed in a pipe using the whole context instead of just the current file being processed.
