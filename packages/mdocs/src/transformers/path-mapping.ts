...
export const pathMappingTransformer: TransformerFactory<
  PathMappingTransformerOptions
> = (options) => {
  const regexMappings = (options?.mappings || []).map((mapping) => ({
    from: new RegExp(mapping.from),
    to: mapping.to,
  }));
  ...
  return async (file, context) => {
    ...
    return {
      ...file,
      output: file.output.replace(relativeInput, relativeOutput),
    };
  };
};
