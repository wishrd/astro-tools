...
export function withTransferState(
  callback: MiddlewareNext,
): Promise<{ response: Response; getTransferState: () => TransferState }> {
  return new Promise((resolve, reject) =>
    stateStorage.run({}, async () => {
      try {
        ...
        resolve({
          response,
          getTransferState: () => {
            return Object.keys(serverState).reduce<TransferState>(
              (state, key) => {
