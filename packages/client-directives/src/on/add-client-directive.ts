...
export function addClientDirective(name: string, loader: ClientDirectiveLoader): void {
  const store = getClientDirectivesStore();
  if (store.has(name)) {
    throw new Error(`client:on custom directive with name "${name}" already registered!`);
  }
